<!DOCTYPE html>
<html lang="pt-br">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Meus Favoritos - Púrpura</title>
    <link rel="stylesheet" href="../html/css/style.css">
    <link rel="stylesheet" href="favoritosstyle.css">
    <link rel="shortcut icon" href="img/logo.png" type="image/x-icon">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
    <link rel="shortcut icon" href="css/img/logo.png" type="image/x-icon">
</head>
<body>
    <header>
        <div class="logo-container">
            <h1><a href="../html/index.php">PÚRPURA</a></h1>
            <p class="tagline">Brilhe em Púrpura</p>
        </div>
        <nav>
            <ul class="menu">
                <li><a href="../html/index.php" class="active">Início</a></li>
                <li><a href="../feminina_controller/femenina.php">Mulher</a></li>
                <li><a href="../homem_controller/homem.php">Homem</a></li>
                <li><a href="../acessorios_controller/acessorio.php">Acessórios</a></li>
                <li><a href="../produtos_controller/produtos.php">Produtos</a></li>
                <li><a href="../html/sobre.php">Sobre</a></li>
            </ul>
        </nav>
        <div class="icons">
            <a href="../produtos_controller/produtos.php" class="icon"><i class="fas fa-search"></i></a>
            <a href="../conta_controller/conta.php" class="icon"><i class="fas fa-user"></i></a>
            <a href="../favoritos_controller/favoritos.php" class="icon"><i class="fas fa-heart"></i></a>
            <a href="../carrinho_controller/carrinho.php" class="icon cart-icon"><i class="fas fa-shopping-bag"></i><span class="cart-count">0</span></a>
        </div>
    </header>

    <section class="page-header">
        <div class="container">
            <h1>Meus Favoritos</h1>
            <div class="breadcrumb">
                <a href="index.html">Início</a> / <span>Favoritos</span>
            </div>
        </div>
    </section>

    <section class="favoritos-section">
        <div class="container">
            <div class="favoritos-header">
                <h2>Seus Itens Favoritos <span class="favoritos-count">(4)</span></h2>
                <div class="favoritos-actions">
                    <button class="btn-secondary"><i class="fas fa-share-alt"></i> Compartilhar Lista</button>
                    <button class="btn-secondary delete-all"><i class="fas fa-trash"></i> Limpar Tudo</button>
                </div>
            </div>

            <div class="favoritos-empty" style="display: none;">
                <div class="empty-icon">
                    <i class="far fa-heart"></i>
                </div>
                <h3>Sua lista de favoritos está vazia</h3>
                <p>Adicione produtos à sua lista de favoritos para vê-los aqui</p>
                <a href="produtos.html" class="btn-primary">Explorar Produtos</a>
            </div>

            <div class="favoritos-grid">
                <div class="favorito-card">
                    <div class="favorito-actions">
                        <button class="remove-btn"><i class="fas fa-times"></i></button>
                    </div>
                    <div class="favorito-image">
                        <img src="/api/placeholder/300/400" alt="Produto 1">
                        <span class="tag new">NOVO</span>
                    </div>
                    <div class="favorito-info">
                        <h3>Vestido Púrpura Elegance</h3>
                        <p class="favorito-category">Vestidos</p>
                        <p class="favorito-price">R$ 199,90</p>
                        <div class="favorito-buttons">
                            <button class="btn-primary add-to-cart"><i class="fas fa-shopping-bag"></i> Adicionar ao Carrinho</button>
                        </div>
                    </div>
                </div>

                <div class="favorito-card">
                    <div class="favorito-actions">
                        <button class="remove-btn"><i class="fas fa-times"></i></button>
                    </div>
                    <div class="favorito-image">
                        <img src="/api/placeholder/300/400" alt="Produto 2">
                    </div>
                    <div class="favorito-info">
                        <h3>Camiseta Black Purple</h3>
                        <p class="favorito-category">Camisetas</p>
                        <p class="favorito-price">R$ 89,90</p>
                        <div class="favorito-buttons">
                            <button class="btn-primary add-to-cart"><i class="fas fa-shopping-bag"></i> Adicionar ao Carrinho</button>
                        </div>
                    </div>
                </div>
                
                <div class="favorito-card">
                    <div class="favorito-actions">
                        <button class="remove-btn"><i class="fas fa-times"></i></button>
                    </div>
                    <div class="favorito-image">
                        <img src="/api/placeholder/300/400" alt="Produto 3">
                        <span class="tag sale">-20%</span>
                    </div>
                    <div class="favorito-info">
                        <h3>Calça Violet Comfort</h3>
                        <p class="favorito-category">Calças</p>
                        <p class="favorito-price"><span class="old-price">R$ 159,90</span> R$ 127,92</p>
                        <div class="favorito-buttons">
                            <button class="btn-primary add-to-cart"><i class="fas fa-shopping-bag"></i> Adicionar ao Carrinho</button>
                        </div>
                    </div>
                </div>
                
                <div class="favorito-card">
                    <div class="favorito-actions">
                        <button class="remove-btn"><i class="fas fa-times"></i></button>
                    </div>
                    <div class="favorito-image">
                        <img src="/api/placeholder/300/400" alt="Produto 4">
                    </div>
                    <div class="favorito-info">
                        <h3>Jaqueta Modern Purple</h3>
                        <p class="favorito-category">Jaquetas</p>
                        <p class="favorito-price">R$ 249,90</p>
                        <div class="favorito-buttons">
                            <button class="btn-primary add-to-cart"><i class="fas fa-shopping-bag"></i> Adicionar ao Carrinho</button>
                        </div>
                    </div>
                </div>
            </div>

            <div class="favoritos-footer">
                <div class="recomendados">
                    <h3>Recomendados para você</h3>
                    <div class="recomendados-grid">
                        <div class="product-card">
                            <div class="product-image">
                                <img src="/api/placeholder/300/400" alt="Produto Recomendado 1">
                                <div class="product-overlay">
                                    <a href="#" class="overlay-icon"><i class="fas fa-heart"></i></a>
                                    <a href="#" class="overlay-icon"><i class="fas fa-shopping-cart"></i></a>
                                    <a href="#" class="overlay-icon"><i class="fas fa-search"></i></a>
                                </div>
                            </div>
                            <div class="product-info">
                                <h3>Blusa Púrpura Chic</h3>
                                <p class="product-category">Blusas</p>
                                <p class="product-price">R$ 129,90</p>
                            </div>
                        </div>
                        
                        <div class="product-card">
                            <div class="product-image">
                                <img src="/api/placeholder/300/400" alt="Produto Recomendado 2">
                                <div class="product-overlay">
                                    <a href="#" class="overlay-icon"><i class="fas fa-heart"></i></a>
                                    <a href="#" class="overlay-icon"><i class="fas fa-shopping-cart"></i></a>
                                    <a href="#" class="overlay-icon"><i class="fas fa-search"></i></a>
                                </div>
                                <span class="tag new">NOVO</span>
                            </div>
                            <div class="product-info">
                                <h3>Saia Midi Elegance</h3>
                                <p class="product-category">Saias</p>
                                <p class="product-price">R$ 159,90</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>

    <section class="newsletter">
        <div class="newsletter-content">
            <h2>Inscreva-se na nossa newsletter</h2>
            <p>Receba as novidades, promoções exclusivas e dicas de moda em primeira mão</p>
            <form class="newsletter-form">
                <input type="email" placeholder="Seu e-mail" required>
                <button type="submit" class="btn-primary">Inscrever-se</button>
            </form>
        </div>
    </section>

    <footer>
        <div class="footer-content">
            <div class="footer-column">
                <h3>Púrpura</h3>
                <p>Sua essência em cores, moda que reflete sua personalidade.</p>
                <div class="social-media">
                    <a href="#" class="social-icon"><i class="fab fa-facebook-f"></i></a>
                    <a href="#" class="social-icon"><i class="fab fa-instagram"></i></a>
                    <a href="#" class="social-icon"><i class="fab fa-twitter"></i></a>
                    <a href="#" class="social-icon"><i class="fab fa-pinterest"></i></a>
                </div>
            </div>
            <div class="footer-column">
                <h3>Links Rápidos</h3>
                <ul class="footer-links">
                    <li><a href="#">Início</a></li>
                    <li><a href="#">Produtos</a></li>
                    <li><a href="#">Sobre nós</a></li>
                    <li><a href="#">Contato</a></li>
                    <li><a href="#">Blog</a></li>
                </ul>
            </div>
            <div class="footer-column">
                <h3>Ajuda</h3>
                <ul class="footer-links">
                    <li><a href="#">Dúvidas Frequentes</a></li>
                    <li><a href="#">Envios e Entregas</a></li>
                    <li><a href="#">Política de Devoluções</a></li>
                    <li><a href="#">Termos e Condições</a></li>
                    <li><a href="#">Política de Privacidade</a></li>
                </ul>
            </div>
            <div class="footer-column">
                <h3>Contato</h3>
                <ul class="contact-info">
                    <li><i class="fas fa-map-marker-alt"></i> Av. Principal, 1000</li>
                    <li><i class="fas fa-phone"></i> (11) 99999-9999</li>
                    <li><i class="fas fa-envelope"></i> contato@purpura.com.br</li>
                </ul>
                <div class="payment-methods">
                    <i class="fab fa-cc-visa"></i>
                    <i class="fab fa-cc-mastercard"></i>
                    <i class="fab fa-cc-amex"></i>
                    <i class="fab fa-cc-apple-pay"></i>
                </div>
            </div>
        </div>
        <div class="copyright">
            <p>&copy; 2025 Púrpura - Todos os direitos reservados.</p>
        </div>
    </footer>
</body>
</html>